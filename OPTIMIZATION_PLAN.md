...
| 已完成 | 建立统一错误分类 | 本次提交 | 已覆盖认证、网络、提取码、HTTP 429、链接失效、下载错误及六个平台关键协议变化；未知平台业务码保持原错误，不做文案猜测 |
| 已完成 | 统一网络客户端配置 | 本次提交 | 控制面和下载面使用独立 Dispatcher/连接池；同步分片请求增加全局 64 连接许可，替换无效的 512 Dispatcher 配置 |
| 已完成 | 分享目录增量分页 | 本次提交 | 六个平台分享浏览均已增量追加；123 正确区分空字符串 next（继续）与 null（结束），递归下载仍获取完整目录 |
| 已完成 | 统一批量任务调度 | 本次提交 | 解析页及六个平台个人网盘的文件夹/多选下载均使用串行 `BatchTaskRunner`，统一进度、取消、失败继续和结果统计 |
| 已完成 | 降低下载列表高频重组 | 本次提交 | 下载页不再订阅整张实时统计 Map；根任务卡和文件夹子任务按 ID 独立订阅并去重，其他任务速度变化不触发当前卡片 |
| 已完成 | 清理启动阶段工作 | 本次提交 | 数据库状态修复优先；持久化清理与夸克兜底扫描在后台依次执行，增加总超时、单条超时、页数和目录数量上限 |
