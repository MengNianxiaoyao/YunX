...
| 已完成 | 加密持久化下载请求头 | `b71a550` | 请求头通过 Android Keystore 加密入库，旧明文任务读取后自动迁移，任务删除时释放内存缓存 |
| 已完成 | 普通文件 SHA-256 完整性校验 | `e300db7` | 任务提供 SHA-256 时在保存前流式校验，不匹配归类为完整性失败且不保存文件；无哈希时保留长度校验 |
| 已完成 | 统一个人网盘 `CloudFileSource` 入口 | `21dc936`–`5db4cb4` | 六个平台个人网盘的列表、取链、文件操作和分享均已迁移到各自 FileSource，ViewModel 不再直接调用平台 API |
| 已完成 | 建立统一错误分类 | `a85f00e`–`cec0fe5` | 已覆盖认证、网络、提取码、HTTP 429、链接失效、下载错误及六个平台关键协议变化；未知平台业务码保持原错误，不做文案猜测 |
| 已完成 | 统一网络客户端配置 | `7b0a48f` | 控制面和下载面使用独立 Dispatcher/连接池；同步分片请求增加全局 64 连接许可，替换无效的 512 Dispatcher 配置 |
| 已完成 | 分享目录增量分页 | `7f62117`–`a6656a9` | 六个平台分享浏览均已增量追加；123 正确区分空字符串 next（继续）与 null（结束），递归下载仍获取完整目录 |
| 已完成 | 统一批量任务调度 | `b40d814`、`a6af6c1`、`5cf0765` | 解析页及六个平台个人网盘的文件夹/多选下载均使用串行 `BatchTaskRunner`，统一进度、取消、失败继续和结果统计 |
| 已完成 | 降低下载列表高频重组 | `a8b0747` | 下载页不再订阅整张实时统计 Map；根任务卡和文件夹子任务按 ID 独立订阅并去重，其他任务速度变化不触发当前卡片 |
...
| 已完成 | 下载业务域文案资源化 | `262f320` | 下载页、任务状态、前台通知、直链弹窗、手动添加、文件打开/安装及后台下载引导已迁移；动态文件信息和持久化错误详情保持业务数据 |
| 已完成 | 解析业务域文案资源化 | `8a47977`、`3c6b563` | 解析页面、批量进度、错误 Banner、转存/取链/批量结果和收藏反馈已迁移；Unknown 错误使用安全 fallback，不展示原始异常消息 |
| 已完成 | 设置业务域文案资源化 | `61440b7`、`8675355`、`fb83595`、`53e0990` | 主题、更新、日志导出、账号备份、下载参数、电池优化、关于和支持页面已迁移 |
| 部分完成 | 共享网盘浏览文案资源化 | `3dbc437`、`40bb026`、`8312a0a`、`d9a67a4`、`e400a6c`、`bcbb5e4`、`bde0a36`、`7a3113a`、待提交 | `CloudBrowserScreen`、加载更多、多选、分页、删除确认、处理中 UI、`CloudAccountSheet`、`CloudActionSheets`、`CloudSaveSheet`、`CloudDriveScreen`、`DriveScreen` 账号卡片、五个平台共享浏览/分享页面、`CloudFileSheets`（含批量操作和分享结果）、收藏页、六个平台登录页、`OnboardingScreen`、`GitHubCard`、`ScrollToTopButton` 及设置页通知详情和线程数已迁移；英文资源与 CI 硬编码扫描仍待建设 |
...
### 6.1 以 `CloudFileSource` 作为个人网盘业务唯一入口【已完成】
...
### 6.4 减少 `ResolveViewModel` 中的路由分支【已完成】
...
当前已为下载任务和分享解析关键链路增加脱敏本地指标：

- 自动重试记录任务 ID、平台、当前重试次数、配置上限、失败分类和累计耗时。
- 任务成功、最终失败或取消时只记录一次终态指标。
- 耗时使用 `System.nanoTime()` 单调时钟，避免系统时间变化影响结果。
- 平台仅允许已知标识，未知值归一为 `generic`。
- 指标不包含 URL、文件名、请求头、凭证、保存路径和原始异常消息。
- 分享解析覆盖初次解析、目录列表、追加分页和单项/批量取链，统一记录成功、失败或取消终态。
- 初次解析将会话创建与根目录加载视为一个完整操作，不重复统计目录指标。
- 分享操作失败只记录稳定错误分类，未知服务端诊断消息不进入指标。

本项只提供脱敏本地日志，不包含远程埋点或用户身份标识。Phase 4 的 operation ID 和请求阶段上下文日志已完成，详见 8.2。
...
- 解析会话、根目录、目录列表、追加分页和取链请求共享各自逻辑操作的随机 `resolve-*` ID。
- 下载单次运行及自动重试共享随机 `download-*` ID，失败请求可与重试和终态指标关联。
- API 与下载客户端通过统一拦截器记录平台、阶段、HTTP 状态、重试次数、单调时钟耗时和稳定错误类型。
- 下载成功分片不逐条记录，避免大文件下载产生日志洪泛。
- ID、平台、阶段和错误类型均使用白名单格式；不读取 URL、Host、请求头、响应体和异常消息。
- 解析与下载旧日志已移除文件名、保存路径、URL、请求头名称及原始异常信息。

本项已完成。共享客户端请求拦截器、operation ID 持久化、暂停恢复和进程重建复用、暂停与删除终态语义均已实现并完成对应测试；更广泛的日志审计和 DownloadManager 集成测试仍作为独立工程任务推进。
...
当前 `strings.xml` 只有 `app_name`，全仓库没有 `stringResource` 引用；UI 源码规模较大，用户可见文案广泛硬编码。这会导致文案修改容易漏改、无法支持多语言、动态文本缺少统一格式约束，以及无法通过资源工具发现重复或未翻译文案。
...
2. 下载页、下载状态、下载通知、进度、速度和任务数量。【已完成：`262f320`】
3. 解析页、链接提示、解析错误和批量操作进度。【已完成：`8a47977`、`3c6b563`】
4. 设置页、备份、更新和电池优化提示。【已完成：`61440b7`、`8675355`、`fb83595`、`53e0990`】
...
1. 持久化临时清理任务。【已完成：`616c914`】
2. 抽取并测试下载状态机。【已完成：`b07c213`】
3. 完善下载失败和保存失败路径。【已完成：`a32fdee`】
4. 统一敏感日志脱敏。【已完成：`9c35356`】
5. 增加 Room Migration 和 DownloadManager 回归测试。【部分完成：`d619093`、`0eb5da4`、`5f6f69e`；DownloadManager 集成测试仍待建设】
...
1. 六平台分享目录分页增量化。【已完成：`7f62117`–`a6656a9`】
2. 分离控制面和下载面 OkHttp 客户端。【已完成：`7b0a48f`】
3. 抽取批量任务调度器。【已完成：`b40d814`、`a6af6c1`、`5cf0765`】
4. 优化下载列表的高频重组。【已完成：`a8b0747`】
5. 增加耗时和重试指标。【已完成：`c951e3b`、`d9e95da`】
...
2. 迁移公共导航、操作和状态文案。
3. 迁移下载、解析、设置和平台页面。【已完成中文默认资源迁移：下载 `262f320`、解析 `8a47977`/`3c6b563`、设置 `61440b7`/`8675355`/`fb83595`/`53e0990`、共享浏览 `3dbc437`/`40bb026`/`8312a0a`/`d9a67a4`/`e400a6c`/`bcbb5e4`/`bde0a36`/`7a3113a`/待提交（含五个平台分享页、`CloudFileSheets`、收藏页、六个平台登录页、`OnboardingScreen`、`GitHubCard`、`ScrollToTopButton` 及设置页通知详情和线程数）；剩余英文资源与 CI 硬编码扫描】
4. 处理动态数量和格式化文案。【部分完成：下载和解析页面使用格式资源与 plurals】
5. 增加英文资源并完成多语言回归。
...
当前可靠性、平台边界、性能和请求上下文治理的主要任务已完成。下一阶段优先推进文案资源化、DownloadManager 集成测试基础设施和剩余凭证类型迁移；文案资源化从公共导航和操作开始，按页面拆分提交，避免一次性修改全部 UI 源码。
