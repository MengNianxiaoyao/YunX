...
| 部分完成 | 增加耗时和重试指标 | `c951e3b` | 下载任务记录脱敏的重试事件及成功、失败、取消终态，使用单调时钟统计耗时；解析和取链指标仍待补充 |
...
后续仍需补充分享解析、目录列表和获取直链耗时，完成后再将本项标记为已完成。
...
5. 增加耗时和重试指标。【部分完成：`c951e3b`；解析和取链指标待补充】
