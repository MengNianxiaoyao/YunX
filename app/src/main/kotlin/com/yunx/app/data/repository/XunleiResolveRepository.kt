...
class XunleiResolveRepository(
    ...
    private suspend fun credential(): CloudCredential.Xunlei =
        credentialProvider() ?: throw IllegalStateException("请先登录迅雷网盘")

    /** 取 access_token 并缓存 user_id（captcha/init 需要，空 user_id 会得到降级 token） */
    private suspend fun access(): CloudCredential.Xunlei {
        ensureFreshToken()   // token 过期则先自动刷新
        return credential().also { api.cacheUserId(it.accessToken) }
    }

    /** token 即将过期（<60s）或已过期时，用 refresh_token 自动刷新（导入恢复后旧 token 过期也适用） */
    private suspend fun ensureFreshToken() {
        val acc = credentialProvider()?.accessToken ?: return
    ...
    override suspend fun createSession(link: String, pwd: String?, cookie: String): Result<ShareSession> =
        runCatching {
            ...
            val result = api.getShare(shareId, effectivePwd, credential)
            ...
            do {
                val next = if (dirFid.isBlank() || dirFid == "0") {
                    val page = api.getShare(
                        session.shareId, passCodes[session.shareId] ?: "", credential, pageToken
                    ) ?: throw IllegalStateException("未获取到文件列表")
                    files += page.files
                    page.nextPageToken
                } else {
                    val page = api.getShareDetail(
                        session.shareId, dirFid, session.stoken, credential, pageToken
                    ) ?: throw IllegalStateException("未获取到文件列表")
                    files += page.files
                    page.nextPageToken
                }
    ...
    override suspend fun getShareDownloadLink(
        ...
        runCatching { api.batchDelete(listOf(savedFid), access()) }
        link
    }.fold(
        onSuccess = { Result.success(it) },
        onFailure = { Result.failure(it) }
